...
function processarInformacio() {
	$.ajax({
		//Obtenim les dades dels inmobles de compra
		url: 'http://localhost:8080/dawInmoExpress/inmoble/venda',
		dataType: 'json',
...
function processarDada(dada) {
	...
	'</a></h4><p class="card-text"><strong>Tipus :</strong> '+ dada.tipo +'</p><p class="card-text"><strong>Ubicació :</strong> '+ dada.ubicacion +
	'</p><p class="card-text"><strong>Preu :</strong> '+ dada.precio +'€</p></div></div></div>');
	return $item;
}